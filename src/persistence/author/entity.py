...
class AuthorBookEntity(Base):
    ...
    author_id: Union[int, Column] = Column(
        ForeignKey("author.id", ondelete="RESTRICT"), primary_key=True
    )
...
class AuthorEntity(Base):
    ...
    def book_ids(self, books: List[int]):
        self.r_book_ids = list(
            map(lambda _id: AuthorBookEntity(author_id=self.id, book_id=_id), books)
        )
